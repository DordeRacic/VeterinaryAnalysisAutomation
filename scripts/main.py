...
# === HELPERS ===
def label_from_id(_map: dict, _id, default: str = "") -> str:
    """Invert the mapping safely to get the label for a given id."""
    inv = {v: k for k, v in _map.items()}
...
with col2:
    with st.container(border=True):
        ...
        breed_options = sorted(breed_map.keys())  # avoid blank default
...
if submit_button:
    ...
    if all_valid:
        ...
        # DEBUG once, before POST
        st.write("DEBUG IDs", {"species_id": species_id, "breed_id": breed_id, "sex_id": sex_id})

        try:
            response = requests.post(PATIENT_ADD_URL, headers=headers, json=payload, timeout=20)
